...
#%%
def _step(double prec_step, double evap_step, double q_old, param, 
          extra_param):
    '''
    prec mm/hr
    evap = mm/hr    
    '''
    cdef double inp, q_sim
...
def simulate(prec, evap, param, extra_param, q_0=INITIAL_Q):
    '''
...
def NSE(x,y,q='def',j=2.0):
    ...
    x = np.array(x)
    y = np.array(y)
    a = np.nansum(np.power(x - y,j))
    b = np.nansum(np.power(y - np.nanmean(y), j))
    F = 1.0 - a/b
    return F
