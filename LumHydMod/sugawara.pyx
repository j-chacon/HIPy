...
INITIAL_Q = 1.0
INITIAL_PARAM = [0.5, 0.2, 0.01, 0.1, 10.0, 20.0, 1.0, 1.0]
...
def _step(double prec, double evap, st, param, extra_param):
    '''
    ...
    '''
    cdef double S1Old, S2Old, S1New, S2New, k1, k2, k3, k4, d1, d2, rfcf, DT
    cdef double Area, H1, q1, q2, q3, Q1, Q2, Q
    # Old states
    S1Old = st[0]
    S2Old = st[1]
...
def simulate(prec, evap, param, extra_param, init_st=INITIAL_STATES, q_0=10):
    '''
...
def calibrate(prec, evap, extra_param, q_rec, bounds=None, verbose=False):
    ...
    cal_res = opt.minimize(mod_wrap, INITIAL_PARAM, bounds=bounds,
                           method='L-BFGS-B')
