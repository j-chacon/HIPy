...
INITIAL_Q = 10
INITIAL_PARAM = [0.5, 0.2, 0.01, 0.1, 10.0, 20.0]
...
def _step(prec, evap, st, param, extra_param):
    ...
    #    EP(1) DT: Number of seconds in the time step [s]
    #    EP(2) AREA: Catchment area [km²]
    #
    #Outputs
    #Q -> Flow [m³/s]
    #S -> Updated system states(2)[S1, S2]
    '''
...
def simulate(all_inp, param, extra_param):
    '''
    ...
    for i in xrange(len(all_inp)):
        step_res = _step(all_inp[i], st[i], param, extra_param)
        q.append(step_res[0])
        st.append(step_res[1])

    return q, st

def calibration(all_inp, extra_param, q_rec):

    def mod_wrap(param_cal):
        q_sim = simulate(all_inp, param_cal, extra_param)[0]
        try:
